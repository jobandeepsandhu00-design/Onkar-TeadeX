  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[1200] flex justify-end bg-black/65 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
      <aside className="flex h-[100dvh] w-full max-w-[520px] flex-col border-l border-cyan-400/15 bg-[#06101f] shadow-[-30px_0_80px_rgba(0,0,0,.6)]">
        ...
        <footer className="flex shrink-0 items-center justify-between border-t border-white/10 p-3 pb-[calc(.75rem+env(safe-area-inset-bottom))] text-[11px]"><button onClick={() => void brainRequest("/notifications/read-all", "POST", {}).then(inbox.refresh)} className="text-cyan-300"><Check size={13} className="mr-1 inline" />Mark all read</button><button onClick={() => void brainRequest("/notifications/read", "DELETE").then(inbox.refresh)} className="text-slate-500"><Trash2 size={13} className="mr-1 inline" />Clear read</button></footer>
      </aside>
    </div>}
  </>;
}
