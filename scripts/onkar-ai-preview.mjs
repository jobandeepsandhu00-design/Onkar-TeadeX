...
const server = await createServer({
  ...
  plugins: [
    tailwind(),
    {
      ...
      load(id) {
        if (id === "\0onkar-ai-preview.tsx")
          return `import React,{useState,useEffect} from 'react';import{createRoot}from'react-dom/client';import{Eye,EyeOff}from'lucide-react';import Workspace from '/src/onkar-ai/OnkarAIWorkspace.tsx';import{OnkarAIAgentCommandCenter}from'/src/onkar-ai/HomeAgentCommandCenter.tsx';import'/src/index.css';function HomePreview({navigate}){const[agentsVisible,setAgentsVisible]=useState(true);return <div style={{maxWidth:590,margin:'auto',padding:'18px 14px 80px'}}><header style={{display:'flex',alignItems:'center',gap:10,marginBottom:18}}><img src="/onkar-tradex-logo.png" style={{width:34}}/><strong style={{fontSize:16,color:'#eef5ff'}}>Onkar TradeX</strong><span style={{marginLeft:'auto',color:'#8093ad',fontSize:11}}>Home preview</span></header><section style={{border:'1px solid #5f451e',borderRadius:16,padding:18,marginBottom:14,background:'#151011',color:'#d9a43e'}}><small style={{letterSpacing:3}}>MOOL MANTAR</small><h2 style={{textAlign:'center',margin:'16px 0 4px'}}>Nirbhau</h2></section><section style={{border:'1px solid #26364d',borderRadius:16,padding:18,marginBottom:14,background:'#0b1424',color:'#eef5ff'}}><small style={{letterSpacing:2,color:'#6f829b'}}>ACCOUNT OVERVIEW</small><h3 style={{margin:'12px 0 4px'}}>FTMO 100K</h3><p style={{margin:0,color:'#43d6a0',fontSize:12}}>Account metrics available</p></section><div style={{display:'flex',alignItems:'center',gap:10,color:'#6f829b',fontSize:10,fontWeight:800,letterSpacing:2,margin:'8px 0'}}><span>ONKAR AI MULTI-AGENT COMMAND CENTER</span><span style={{height:1,background:'#1c2d42',flex:1}}/><button aria-label={agentsVisible?'Hide Onkar AI Multi-Agent Command Center':'Show Onkar AI Multi-Agent Command Center'} onClick={()=>setAgentsVisible(v=>!v)} style={{border:0,background:'transparent',color:agentsVisible?'#718399':'#2bbcff',padding:4}}>{agentsVisible?<Eye size={14}/>:<EyeOff size={14}/>}</button></div>{agentsVisible&&<div className="oai"><OnkarAIAgentCommandCenter onNavigate={navigate}/></div>}<section style={{border:'1px solid #1e344c',borderRadius:16,padding:18,marginTop:14,background:'#08111e',color:'#72c8ff'}}><small style={{letterSpacing:2}}>MARKET OVERVIEW</small><h3>London + New York Session</h3><p style={{color:'#667d98',fontSize:11}}>Existing dashboard content continues here unchanged.</p></section></div>}function Preview(){const[path,setPath]=useState(location.pathname);useEffect(()=>{const sync=()=>setPath(location.pathname);addEventListener('popstate',sync);return()=>removeEventListener('popstate',sync)},[]);const navigate=p=>{history.pushState({},'',p);setPath(p);scrollTo(0,0)};return path.startsWith('/onkar-ai')?<Workspace path={path} onNavigate={navigate} onExit={()=>navigate('/')} accountName="Preview account"/>:<HomePreview navigate={navigate}/>};createRoot(document.getElementById('root')).render(<Preview/>);`;
      },
